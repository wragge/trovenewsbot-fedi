...
def get_url(query):
    if match := re.search(r"https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_\\+.~#?&\\/=]*)", query):
...
def process_toot(toot_json):
    toot = json.loads(toot_json)
    toot_id = toot['id']
    # user = tweet['user']['screen_name']
    query, sort, random, illustrated, article_id, category, hello = parse_toot(toot)
    if hello is True:
    ...
    send_toot(article, message, toot, illustrated=illustrated)
    time.sleep(20)
...
def parse_toot(toot):
    ...
    query = soup.get_text().strip().lower()
...
def send_toot(article, message=None, toot=None, illustrated=False):
    if article:
        ...
        if '@abcnews' in message:
            ...
            message_length = len(message) + len(toot["account"]["acct"]) + 2
        else:
            message_length = len(message)
        chars = 500 - (message_length + len(article_date) + len(newspaper) + 10)
    ...
    print(status)
    if toot:
        # Have to convert the dict back into the object that mastodon.py expects
        toot["account"] = AttribAccessDict(toot["account"])
        toot = AttribAccessDict(toot)
        # print(toot.account.acct)
        # pass
        mastodon.status_reply(to_status=AttribAccessDict(toot), status=status, media_ids=media, untag=True, visibility="unlisted")
        #api.update_status('@{} {}'.format(user, status), media_ids=media_ids, in_reply_to_status_id=tweet_id)
    else:
        # pass
        mastodon.status_post(status, media_ids=media, visibility="public")
...
def random_toot(option=None):
    article, message, illustrated = random_article(option=option)
    send_toot(article, message=message, illustrated=illustrated)
...
def get_page_thumbnail(article_id, size, illustrated=False):
    ...
    # Path('page-image.jpg').write_bytes(response.content)
...
def reply_guardian():
    ...
    # Get second article as first often goes to blog
    ...
    print(redact)
    if redact is False:
        latest_url = news.entries[1].link
        try:
        ...
        if latest_url != last_url:
            ...
            if article:
                ...
                send_toot(article, message=message)
...
if __name__ == '__main__':
    ...
    if args.task == 'random':
        ...
        random_toot(option='onthisday')
    elif args.task == 'guardian':
        reply_guardian()
    elif args.task == 'test':
        test()
